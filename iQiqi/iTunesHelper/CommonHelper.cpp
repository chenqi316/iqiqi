...
void CommonHelper::DeleteLocalFolder(char* localPath,int deleteAll){
// 	if (deleteAll)
// 	{
...
int CommonHelper::LocalFolderExits(char* localPath){
	return _access(localPath,0);
}

void CommonHelper::GetCFString(HANDLE hValue,WCHAR* charstring,UINT charstringLen)
{
	...
	if (retID == retstring)
	{
		...
		iTunesApi::CFStringGetCString(hValue, charstring, charstringLen, kCFStringEncodingUnicode);
	}
}
